...
void main() {
   ...
   //UART1.Write('A');                           //MCU'dan PC ye karakter gönderiliyor.
   delay_ms(100);
   ...
   while(1)
   {
   if(UART1_DATA_READY())                       //Bir data geldiðinde bu koþul iþleyecek
   {
    UART1_Read_Text(uart_rd,";",6);             //PC den MCU'ya gönderilen string okunuyor.
    delay_ms(100);

    if(strcmp(uart_rd,"led,1")==0)              //string karþýlatýrma yapýlýyor.
      {
       led=1;
       delay_ms(100);
      ...
      if(strcmp(uart_rd,"led,0")==0)
      {
       ...
       buffer_sifirlama();
        ...
        if(strcmp(uart_rd,"led,?")==0)         //Ledin durumunun öðrenilmesi için koþul bölümü.
      {
          if(led==1)
          {
